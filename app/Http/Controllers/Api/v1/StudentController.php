...
class StudentController extends Controller
{
    ...
    public function index(StudentRequest $request)
    {
        ...
        $student = Students::with('education_facilities');
        ...
        if (isset($requestedData['search'])) {
            ...
            $student->where(function ($query) use ($search) {
                $query->where('education_facility_id', 'LIKE', "%{$search}%")
        ...
        $student = $requestedData['paginate']
            ? $student->paginate($requestedData['paginate'])
            : $student->get();
        return $this->sendResponse(
            [
